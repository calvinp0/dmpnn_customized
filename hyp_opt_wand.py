...
def objective(train_mcd, val_mcd, test_mcd, unscale, pt_ea, wandb_name, trial):
    ...
    wandb.init(project=wandb_name, mode="offline",
    ...
    ckpt = pl.callbacks.ModelCheckpoint(monitor='val/rmse', mode='min', save_top_k=1)

    trainer = pl.Trainer(
        accelerator="auto", devices=1, max_epochs=trial.suggest_int('epochs',20,120,step=10),
        logger=False, enable_progress_bar=False,
        callbacks=[ckpt, pl.callbacks.EarlyStopping(monitor='val/rmse',
                                                    patience=8,mode='min')])
    trainer.fit(model, tl, vl)
    # ─── lightning-side test metrics ──────────────────────────────────────────
    ...
    # store the Lightning checkpoint path
    trial.set_user_attr("ckpt", ckpt.best_model_path)

    # ─── custom post-processing on *unscaled* targets ────────────────────────
    model.eval()
    with torch.no_grad():
        scaled_preds_batches = trainer.predict(model, dataloaders=xl)
    scaled_preds = torch.cat(scaled_preds_batches, dim=0).cpu().numpy()

    #  inverse-transform: first StandardScaler → then Yeo-Johnson
    ...
    pct_err_A = 100 * mae_vals[0] / A_true.mean()
    ...
    wandb.log({
        ...
        "r2_logA":    r2_logA,
        "pct_err_A":  pct_err_A,
        # ranges (quick sanity-check)
        "range_A_min":  A_pred.min(),  "range_A_max":  A_pred.max(),
        "range_n_min":  n_pred.min(),  "range_n_max":  n_pred.max(),
        "range_Ea_min": Ea_pred.min(), "range_Ea_max": Ea_pred.max(),
    })
